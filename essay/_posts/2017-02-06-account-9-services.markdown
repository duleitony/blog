...
每个支付应用可以用的支付方式是不一样的。比如说，扫码收银，可能仅支持微信和支付宝。POS支付，仅支持银行卡。而信用卡返款，只能从其他的借记卡上去扣款。 支付应用的设计和公司的业务有关，并需要考虑在公司业务场景下的用户支付体验。目前应用最全的数支付宝，可以参观下当前已有的支付相关应用：
[![支付宝](http://blog.lixf.cn/img/in-post/arch-product.png)](http://blog.lixf.cn/img/in-post/arch-product.png)
在这个应用中，将支付方式呈现给用户时，哪个方式应该排在前面，这在支付系统中，是通过**引导路由**来实现的。 
...
基于权重的路由，是从各个维度来对通道做建模，分配权重。在执行路由时，计算输入的支付方式和各个通道之间的矢量积，即权重，选择结果最大的，即匹配度最好的通道。 
