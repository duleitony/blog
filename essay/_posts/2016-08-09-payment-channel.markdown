...
基本上和银行对接都需要专线，那么这事情怎么玩呢？
首先，和银行商务、技术负责人沟通， 一般银行的需求都比较强势，需要做好心理准备。 
1. 专线类型， 如果银行没有强势指定专线类型，则推荐使用SDH 2M。常见的就MSTP和SDH两种，都是以太网电口接入，MSTP费用较低，可靠性稍差。
2. 接入点， 一般寻找公司就近的IDC来接入， 联通和电信均可。 
3. 加密机， 有些银行要求使用加密机， 比如中行。此处加密机的作用是：MAC加密（完整性）；支付会话\密码加密（安全性）；密钥交换加密（防截取）。加密机需要提前采购。
4. 跟银行方申请前置机IP网段，以及获知对方与公司通信的前置机IP网段。这两组IP都是NAT后的IP，银行分配给的真实的前置机IP经过最外端的网络防火墙转换后的IP段，后者也是对方的真实前置机IP经过转换后的IP段。 出于安全考虑，双方都不会将真实IP暴露出去，所以要NAT。
