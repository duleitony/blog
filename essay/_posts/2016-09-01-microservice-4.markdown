...
一、支付网关前置应保持稳定，尽量减少系统重启等操作对业务方的影响。为此，我们在支付网关前置前端增加Nginx的LBS网关，专门用于请求转发。当服务器需要重启时，首先从LBS中摘取，重启后再挂接上。
二、支付网关前置的性能问题，通过各个后续尽量异步化来处理。 主流程仅保留记录流水、 执行风控、调用支付路由、调用支付渠道服务，其他操作都异步化处理。
三、接口的安全性问题，包括通道安全和防篡改机制。 通道安全通过HTTPS/SSL来保证，防篡改则通过接口参数签名来处理。现在主流的签名是对接口参数按照参数名称排序后，做加密和散列，参考[微信的签名规范]{https://pay.weixin.qq.com/wiki/doc/api/app/app.php?chapter=4_3}。
...
![Image of Data Flow](http://blog.lixf.cn/img/in-post/arch_risk.png)
